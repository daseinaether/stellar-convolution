...
Post-processing script for calculating binary compact object merger rates.
...
# ====== Global Constants ======
DEFAULT_COSMOLOGY: str = ''
DEFAULT_DCO_TYPE: str = 'BBH'

DEFAULT_MAX_REDSHIFT: float = 10.0
DEFAULT_MAX_REDSHIFT_DETECTION: float = 1.0
DEFAULT_STEP_REDSHIFT: float = 0.001

DEFAULT_STAR_FORMATION_REDSHIFT: float = 10.0

DEFAULT_MU0: float = 0.035
DEFAULT_MUZ: float = -0.23
DEFAULT_SIGMA0: float = 0.39
DEFAULT_SIGMAZ: float = 0.0
DEFAULT_ALPHA: float = 0.0

DEFAULT_MIN_LOGZ: float = -12.0
DEFAULT_MAX_LOGZ: float = 0.0
DEFAULT_STEP_LOGZ: float = 0.01

DEFAULT_DETECTOR_SENSITIVITY: str = '01'
DEFAULT_SNR_TRHESHOLD: float = 8.0
DEFAULT_MAX_CHIRP_MASS: float = 300.0
DEFAULT_STEP_CHIRP_MASS: float = 0.1
DEFAULT_MAX_SYMMETRIC_MASS: float = 0.25
DEFAULT_STEP_SYMMETRIC_MASS: float = 0.01
DEFAULT_MAX_SNR: float = 1000.0
DEFAULT_STEP_SNR: float = 0.1

# PARAMETER CLASSES WILL BE MIGRATED TO 'parameters.py'

@dataclass(frozen=True)
class RedshiftArrayParams:
    """
    Parameters for creating redshift arrays.

    Attributes
    ----------
    max_redshift : `float`
        Maximum redshift to use for calculations.
    max_redshift_detection : `float`
        Maximum redshift to calculate detection rates (must be <= max_redshift).
    step_redshift : `float`
        Redshift step size.
    star_formation_redshift : `float`
        Redshift of first star formation.
    """
    max_redshift: float = DEFAULT_MAX_REDSHIFT
    max_redshift_detection: float = DEFAULT_MAX_REDSHIFT_DETECTION
    step_redshift: float = DEFAULT_STEP_REDSHIFT
    star_formation_redshift: float = DEFAULT_STAR_FORMATION_REDSHIFT

@dataclass(frozen=True)
class MetallicityDistributionParams:
    """
    Parameters for creating metallicity distributions.

    Attributes
    ----------
    mu0 : `float`
        Expected value at redshift 0.
    muz : `float`
        Expected value as function of redshift.
    sigma0 : `float`
        Width at redshift 0.
    sigmaz : `float`
        Width as function of redshift.
    alpha : `float`
        Skewness (0 = lognormal).
    """
...
@dataclass(frozen=True)
class MetallicityProbabilityParams:
    """
    Parameters for creating metallicity probabilities.
    
    Attributes
    ----------
    min_logZ : `float`
        Minimum logZ at which to calculate dPdlogZ.
    max_logZ : `float`
        Maximum logZ at which to calculate dPdlogZ.
    step_logZ : `float`
        Size of logZ steps for finding Z range.
    """
    min_logZ: float = DEFAULT_MIN_LOGZ
    max_logZ: float = DEFAULT_MAX_LOGZ
    step_logZ: float = DEFAULT_STEP_LOGZ

@dataclass(frozen=True)
class DetectionProbabilityParams:
    """
    Parameters for determining detection probabilities.

    Attributes
    ----------
    detector_sensitivity : `str`
        Select detector sensitivity: ['design', '01', '03']
    snr_threshold : `float`
        SNR threshold required for detection.
    max_chirp_mass : `float`
        Maximum chirp mass.
    step_chirp_mass : `float`
        Size of chirp mass step.
    max_symmetric_mass : `float`
        Maximum symmetric mass ratio.
    step_symmetric_mass : `float`
        Size of symmetric mass ratio step.
    max_snr : `float`
        Maximum SNR.
    step_snr : `float`
        Size of SNR step.
    """
    detector_sensitivity: str = DEFAULT_DETECTOR_SENSITIVITY
    snr_threshold: float = DEFAULT_SNR_TRHESHOLD
    max_chirp_mass: float = DEFAULT_MAX_CHIRP_MASS
    step_chirp_mass: float = DEFAULT_STEP_CHIRP_MASS
    max_symmetric_mass: float = DEFAULT_MAX_SYMMETRIC_MASS
    step_symmetric_mass: float = DEFAULT_STEP_SYMMETRIC_MASS
    max_snr: float = DEFAULT_MAX_SNR
    step_snr: float = DEFAULT_STEP_SNR

def compute_redshift_params(params: RedshiftArrayParams = RedshiftArrayParams(), cosmology: str = DEFAULT_COSMOLOGY):
    """ 
    Given limits on the redshift, create an array of redshifts, times, distances, and volumes.

    Parameters
    ----------
    params : `RedshiftArrayParams`, optional
    ...
    redshifts = np.arange(0, max_redshift + step_redshift, step_redshift)
    n_redshifts_detection = int(max_redshift_detection / step_redshift)
    ...
    time_first_SF = cosmology.age(star_formation_redshift).to(u.Myr).value
...
def compute_star_formation_rate(redshifts, a = 0.01, b =2.77, c = 2.90, d = 4.70):
    """
...
def compute_metallicity_distribution(
    redshifts,
    min_logZ_COMPAS: float,
    max_logZ_COMPAS: float,
    metallicity_distribution: MetallicityDistributionParams = MetallicityDistributionParams(),
    metallicity_probability: MetallicityProbabilityParams = MetallicityProbabilityParams()
):
    ...
    Args:
        ...
        step_redshift      --> [float]          step used in redshift calculation
...
def compute_formation_and_merger_rates(n_binaries, redshifts, times, time_first_SF, n_formed, dPdlogZ, metallicities, p_draw_metallicity,
    ...
    step_redshift = redshifts[1] - redshifts[0]
    ...
    for i in range(n_binaries):
        ...
        if first_too_early_index > 0:
            ...
            z_of_formation_index = np.ceil(z_of_formation / step_redshift).astype(int)
...
def compute_snr_and_detection_grids(params: DetectionProbabilityParams = DetectionProbabilityParams()):
    """
        ...
        combined with compute_detection_probability() could be replaced by something like
            for i in range(n_binaries):
                detection_probability = selection_effects.detection_probability(COMPAS.mass1[i],COMPAS.mass2[i],
                                            redshifts, distances, GWdetector_snr_threshold, GWdetector_detector_sensitivity)
        if runtime was not important.

        Args:
            detector_sensitivity                    --> [string]         Which detector detector_sensitivity to use: one of ["design", "O1", "O3"]
            snr_threshold                  --> [float]          What SNR threshold required for a detection
            max_chirp_mass                         --> [float]          Maximum chirp mass in grid
            step_chirp_mass                        --> [float]          Step in chirp mass to use in grid
            max_symmetric_mass                        --> [float]          Maximum symmetric mass ratio in grid
            step_symmetric_mass                       --> [float]          Step in symmetric mass ratio to use in grid
            max_snr                        --> [float]          Maximum snr in grid
            step_snr                       --> [float]          Step in snr to use in grid

        Returns:
            snr_grid_at_1Mpc               --> [2D float array] The snr of a binary with masses (chirp_mass, symmetric_mass) at a distance of 1 Mpc
            detection_probability_from_snr --> [list of floats] A list of detection probabilities for different SNRs
    """
    # get interpolator given detector_sensitivity
    interpolator = selection_effects.SNRinterpolator(detector_sensitivity)

    # create chirp mass and symmetric_mass arrays
    Mc_array = np.arange(step_chirp_mass, max_chirp_mass + step_chirp_mass, step_chirp_mass)
    eta_array = np.arange(step_symmetric_mass, max_symmetric_mass + step_symmetric_mass, step_symmetric_mass)
    ...
    snr_array = np.arange(step_snr, max_snr + step_snr, step_snr)
...
def compute_detection_probability(
    ...
    """
        ...
        Args:
            chirp_mass                             --> [list of floats] Chirp mass of binaries in COMPAS
            symmetric_mass                            --> [list of floats] Symmetric mass ratios of binaries in COMPAS
    ...
    for i in range(n_binaries):
        ...
        Mc_shifted = chirp_mass[i] * (1 + redshifts[:n_redshifts_detection])

        # work out the closest index to the given values of symmetric_mass and chirp_mass
        eta_index = np.round(symmetric_mass[i] / step_symmetric_mass).astype(int) - 1
        Mc_index = np.round(Mc_shifted / step_chirp_mass).astype(int) - 1
        ...
        detection_list_index = np.round(snrs / step_snr).astype(int) - 1
...
def compute_detection_rate(
    path: str,
    dco_type: str = DEFAULT_DCO_TYPE,
    merger_output_filename: str = '',
    weight_column: str = '',
    hubble_time_merges: bool = True,
    pessimistic_CEE: bool = True,
    no_RLOF_after_CEE: bool = True,
    redshift_array: RedshiftArrayParams = RedshiftArrayParams(),
    ...
    metallicity_distribution: MetallicityDistributionParams = MetallicityDistributionParams(), 
    metallicity_probability: MetallicityProbabilityParams = MetallicityProbabilityParams(),
    detection_probability: DetectionProbabilityParams = DetectionProbabilityParams(),
    ...
    """
        ...
        Args:
            ...
            hubble_time_merges  --> [bool]   whether to mask binaries that don't merge in a Hubble time
            ...
            step_redshift          --> [float]  Size of step to take in redshift
    ...
    assert step_chirp_mass < max_chirp_mass, "Chirp mass step size must be less than maximum chirp mass"
    assert step_symmetric_mass < max_symmetric_mass, "Symmetric mass ratio step size must be less than maximum symmetric mass ratio"
    assert step_snr < max_snr, "SNR step size must be less than maximum SNR"
    ...
    nonnegative_args = [(max_redshift, "max_redshift"), (max_redshift_detection, "max_redshift_detection"), (m1_min.value, "m1_min"), (m1_max.value, "m1_max"),
                        ...
                        (step_logZ, "step_logZ"), (snr_threshold, "snr_threshold"), (max_chirp_mass, "max_chirp_mass"),
                        (step_chirp_mass, "step_chirp_mass"), (max_symmetric_mass, "max_symmetric_mass"), (step_symmetric_mass, "step_symmetric_mass"), (max_snr, "max_snr"), (step_snr, "step_snr")]
    ...
    COMPAS = compas_data.COMPASData(path, Mlower=m1_min, Mupper=m1_max, m2_min=m2_min, binaryFraction=fbin, suppress_reminder=True)
    COMPAS.set_DCO_mask(types=dco_type, withinHubbleTime=hubble_time_merges, pessimistic=pessimistic_CEE, noRLOFafterCEE=no_RLOF_after_CEE)
    COMPAS.set_COMPAS_data()
    COMPAS.set_stroopwafel_weights(weight_column)
    m1=COMPAS.retreive_COMPAS_vars("BSE_System_Parameters","Mass@ZAMS(1)");
    m2=COMPAS.retreive_COMPAS_vars("BSE_System_Parameters","Mass@ZAMS(2)");
    ...
    COMPAS.compute_star_forming_mass()
    ...
    if max(chirp_masses)*(1+max_redshift_detection) > max_chirp_mass:
    ...
    redshifts, n_redshifts_detection, times, time_first_SF, distances, shell_volumes = compute_redshift_params(max_redshift, max_redshift_detection, step_redshift, star_formation_redshift, cosmology)

    # find the star forming mass per year per Gpc^3 and convert to total number formed per year per Gpc^3
    sfr = compute_star_formation_rate(redshifts, a = aSF, b = bSF, c = cSF, d = dSF) # functional form from Madau & Dickinson 2014
    ...
    if np.log(np.min(COMPAS.initialZ)) != np.log(np.max(COMPAS.initialZ)): # Will perform integral over metallicities
        dPdlogZ, metallicities, p_draw_metallicity = compute_metallicity_distribution(redshifts, min_logZ_COMPAS = np.log(np.min(COMPAS.initialZ)),
    ...
    formation_rate, merger_rate = compute_formation_and_merger_rates(n_binaries, redshifts, times, time_first_SF, n_formed, dPdlogZ,
    ...
    snr_grid_at_1Mpc, detection_probability_from_snr = compute_snr_and_detection_grids(detector_sensitivity, snr_threshold, max_chirp_mass, step_chirp_mass,
                                                                                    max_symmetric_mass, step_symmetric_mass, max_snr, step_snr)

    # use lookup tables to find the probability of detecting each binary at each redshift
    detection_probability = compute_detection_probability(chirp_masses, etas, redshifts, distances, n_redshifts_detection, n_binaries,
                                                        snr_grid_at_1Mpc, detection_probability_from_snr, step_chirp_mass, step_symmetric_mass, step_snr)
...
def append_rates(
    ...
    detector_sensitivity: str = '01',
    dco_type: str = DEFAULT_DCO_TYPE,
    params: MetallicityDistributionParams = MetallicityDistributionParams(),
    append_binned_rates: bool = False,
    redshift_bin_size: float = 0.1,
    cosmology: str = ''
) -> None:
    """
    ...
    n_redshifts : `int`
        Number of redshifts used to calculate rates.
    max_redshift : `float`
        Maximum value of redshift to store data for.
    detector_sensitivity : `str`
        Detector sensitivity used to calculate rates.
    dco_type : `str`
        DCO type used to calculate rates.
    params : `MetallicityDistributionParams`
        Metallicity distribution parameters.
    append_binned_rates : `bool`
        Appends the binned rates.
    redshift_bin_size : `float`
        Determine redshift bin size if apppending binned rates.
    ...
    with h5.File(path, 'r+') as h_new:
        ...
        if append_binned_rates:
            ...
            redshift_bins = np.arange(0, redshifts[-1]+redshift_bin_size, redshift_bin_size)
            ...
            i_per_crude_bin = redshift_bin_size/fine_binsize 
        ...
        rate_list_names      = ['SEED', 'DCOmask', 'redshifts',  'merger_rate','merger_rate_z0', 'detection_rate'+detector_sensitivity]
...
def delete_rates(
    ...
    append_binned_rates: bool = False):
    """
    Deletes the group containing all rate information from the COMPAS output file.
    ...
    with h5.File(path, 'r+') as h_new:
        ...
        new_rate_group = 'Rates_mu0{}_muz{}_alpha{}_sigma0{}_sigmaz{}'.format(params)
        if append_binned_rates:
            new_rate_group  = new_rate_group + '_zBinned'
...
def plot_rates(
    ...
    """
    Plots and returns summary results.

    Parameters
    ----------
    save_dir : `str`
        Path to directory for saving plot.
...
def parse_cli_args():
    ...
    # Options for the redshift evolution and detector detector_sensitivity
    parser.add_argument("--maxz", dest='max_redshift', help="Maximum redshift to use in array", type=float, default=10)
    parser.add_argument("--zSF", dest='star_formation_redshift', help="redshift of first star formation", type=float, default=10)
    ...
    parser.add_argument("--zstep", dest='step_redshift', help="size of step to take in redshift", type=float,
                        default=0.001)
    parser.add_argument("--sens", dest='detector_sensitivity',
                        help="Which detector detector_sensitivity to use: one of ['design', 'O1', 'O3']", type=str, default="O3")
...
def main():
    ...
    start_CI = time.time()
    detection_rate, formation_rate, merger_rate, redshifts, COMPAS = compute_detection_rate(
        args.path,
        dco_type=args.dco_type,
        weight_column=args.weight_column,
        max_redshift=args.max_redshift,
        max_redshift_detection=args.max_redshift_detection,
        step_redshift=args.step_redshift,
        star_formation_redshift=args.star_formation_redshift,
        ...
        step_logZ=0.01,
        max_chirp_mass=300.0, step_chirp_mass=0.1,
        max_symmetric_mass=0.25, step_symmetric_mass=0.01,
        max_snr=1000.0, step_snr=0.1, cosmology=cosmology)
    end_CI = time.time()
    ...
    if args.append_rates:
        n_redshifts_detection = int(args.max_redshift_detection / args.step_redshift)
        append_rates(args.path, detection_rate, formation_rate, merger_rate, redshifts, COMPAS, n_redshifts_detection,
                     maxz=args.max_redshift_detection, detector_sensitivity=args.detector_sensitivity, dco_type=args.dco_type,
                     mu0=args.mu0, muz=args.muz, sigma0=args.sigma0, sigmaz=args.sigmaz, alpha=args.alpha,
                     append_binned_rates=False, redshift_bin_size=0.05, cosmology=cosmology)
